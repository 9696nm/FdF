...
// 				q = quaternion_axis_angle(mx-vars->set.x, my-vars->set.y, vars->mat_arr[my][mx+1]-vars->set.z, M_PI);
// 				q = rotate_vector(vars->qv, q);
// 				my_mlx_pixel_put(&vars->idata, q.x*vars->set.zoom+WIDTH/2, q.y*vars->set.zoom+HEIGHT/2, 0xFF0FF0FF);
// 			}
// 		}
...
void	mlx(char *name, int **mat_arr)
{
	...
	vars.qv = normalize(quaternion_axis_angle(1, 1, 1, 30 * M_PI / 180));
